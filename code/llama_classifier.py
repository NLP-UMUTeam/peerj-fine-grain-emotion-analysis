...
def main(args): 
    ...
    adapter_model = "./results/llamaClassification"
    tokenizer = AutoTokenizer.from_pretrained(adapter_model)
    
    
    config = AutoConfig.from_pretrained(model_path, label2id=label2id, id2label=id2label)
    model = AutoModelForSequenceClassification.from_pretrained(model_path, config=config, torch_dtype=torch.float16)
    ...
    result_df.to_csv(f"{save_path}/llama_lora_results.csv", index=False)
