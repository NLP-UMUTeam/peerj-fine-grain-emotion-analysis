...
def main(args): 
    ...
    tokenizer = AutoTokenizer.from_pretrained("./results/mbart_classification", use_fast = True)
    
    model = AutoModelForSequenceClassification.from_pretrained("./results/mbart_classification", num_labels=len(label2id), ignore_mismatched_sizes = True).to(device)
    ...
    result_df.to_csv("./results/mbart_result.csv", index=False)
