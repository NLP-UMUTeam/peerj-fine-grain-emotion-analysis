...
def preprocess_function(examples):
  result = tokenizer(examples["tweet_clean"], truncation=True, padding="max_length", max_length=512)
...
def train_model(model, train_dataset, eval_dataset, save_path):
    ...
    trainer.save_model(f'{save_path}/BloomLoraClassification')
...
def main(args): 
    ...
    model = BloomForSequenceClassification.from_pretrained(model_path, num_labels=len(label2id), ignore_mismatched_sizes = True)
